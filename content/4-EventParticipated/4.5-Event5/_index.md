...
**- CloudWatch metrics:** Data of the performance of system on AWS or on premise with CloudWatch Agent, intergrate well with EventBridge, Auto Scaling and DevOps workflow
...
- Q: Our project up until now have been purely built with ClickOps, and we are planning to use CDK. Are there any tool that could scan and turn our existing infastructure into CDK or CloudFormation rather than repoducing the infastructure from scratch with IaC? 
- A: Unfornately no, there isn't a tool that can assist with that problem yet, your team is going to have to built the infastructure from scratch again. If there by any chance that you do found a tool that can assist with please share with us too.
...
- A: Guard Duty can be configured to have a threat list containing custom rules to trigger findings upon activities relating the configured mailicious domains or IPs  

This event is also the first time some of the speaker's first time presenting a topic:
- DevOps and Iac sections was well presented 
